...
def generateDataMutate(test):
    ...
    if test is 0:
        l = 0
        m = len(ref_seq)-series_length*100
    else:
        l = len(ref_seq)-series_length*100
...
with tf.Session() as sess:
    ...
    # save_path = saver.save(sess, "testResults/model_small.ckpt")

    saver.restore(sess, "testResults/model_small.ckpt")
    graph = tf.get_default_graph()
    # readRealData()

    _current_state = np.zeros((batch_size, state_size))
    x, y = generateDataMutate(1)
