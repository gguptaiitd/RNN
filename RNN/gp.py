...
from netbuilder import randomize_network
import copy
...
class TournamentOptimizer:
    ...
    def step(self):
        print('\nPopulation sample:')
        for i in range(0, self.population_sz, 1):
            print("type:", self.population[i]['type']['val'], " "
                  "num_layers:", self.population[i]['num_layers']['val'], " "
                  "hidden size:", self.population[i]['layers'][0]['hidden_size']['val'], " "
                  "optimizer:", self.population[i]['optimizer']['val'])
        self.evaluate()
        children = []
