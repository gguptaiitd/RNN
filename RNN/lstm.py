...
unmatched_seq_length = mutSeries_length + 20
...
def generateDataMutate(test):
    ...
    if test is False:
        l = 0
        m = len(ref_seq)-series_length*5000
    else:
        l = len(ref_seq)-series_length*5000
        m = len(ref_seq)-series_length

    loc = random.sample(list(range(l, m)), refSeries)
    for loc in loc:
        l_seq = series_length
...
class LSTM(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        out, hidden = self.rnn(input, (h0, c0))
        output = self.fc(out)
        ...
        # print("Weights RNN IN FORWARD FC", self.rnn.all_weights)
...
correct = 0
pred_list = []
lstm = LSTM(rep_size, state_size, 1)
optimizer = optim.Adam(lstm.parameters(), lr=0.001)
x, y = generateDataMutate(False)
for epoch_idx in range(num_epochs):
    ...
    for batch_idx in range(num_batches):
        ...
        #     pred = softmax(output).data.max(1)[1]
        #     correct = pred.eq(target.data.max(1)[1]).cpu().sum()
        #     accuracy_list.append(float(correct * 100 / batch_size))
        #     plt.subplot(3, 1, 3)
        #     plt.cla()
    ...
    #     pred = softmax(output).data.max(1)[1]
    #     correct = pred.eq(target.data.max(1)[1]).cpu().sum()
    #     correct_list_test.append(float(correct * 104 / batch_size))
    #     print("TesT Accuracy ", float(correct * 100 / batch_size))
    #     print(" Mean TesT Accuracy ", sum(correct_list_test)/len(correct_list_test))
    #     # plt.subplot(2, 3, 3)
    #     # plt.cla()
    # plt.plot(correct_list_test)
x, y = generateDataMutate(True)
for i in range(1, 50):
    r = random.random()
    random.shuffle(x[start_idx], lambda: r)
    random.shuffle(y[start_idx], lambda: r)
    ...
    pred = softmax(output).data.max(1)[1]
    correct = pred.eq(target.data.max(1)[1]).cpu().sum()
    test_accuracy_list.append(float(correct * 10))
    # Compute confusion matrix
    pred_list.append(np.asarray(pred))
    target_list.append(batchY)

target_list_max = np.argmax(target_list, axis=2)
pred_list = np.asarray(pred_list)
cnf_matrix = confusion_matrix(np.reshape(target_list_max, (-1,)), np.reshape(pred_list, (-1,)))
np.set_printoptions(precision=2)
report = classification_report(np.reshape(target_list_max, (-1,)), np.reshape(pred_list, (-1,)))
print(report)
