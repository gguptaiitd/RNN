...
unmatched_seq_length = mutSeries + 10
...
batchX_placeholder = tf.placeholder(tf.float32, [batch_size, series_length, rep_size])
...
W3 = tf.Variable(np.random.rand(batch_size, series_length, num_classes), dtype=tf.float32)
...
for current_input in inputs_series:
    current_input = tf.reshape(current_input, [batch_size, rep_size])
